    ...
    return (
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="p-8 text-center">
          ...
          <p className="mt-3 text-gray-500">Loading cards...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-1">No cards yet</h3>
          <p className="text-gray-500">Add your first credit card using the form above.</p>
        </div>
      </div>
    );
  }
                ...
                >
                    ...
                    <div className="flex items-center gap-4">
                      {card.imageUrl ? (
                        <img
                          src={card.imageUrl}
                          alt={card.name}
                          className="w-16 h-10 object-contain rounded-md border border-gray-100"
                        />
                      ) : (
                      ...
                      <div className="min-w-0">
                        <p className="font-medium text-gray-900 truncate max-w-xs">{card.name}</p>
                        <p className="text-sm text-gray-500 truncate max-w-xs">{card.slug}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`font-medium ${card.annualFee === 0 ? 'text-green-600' : 'text-gray-900'}`}>
                      {card.annualFeeText}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${
                      card.creditRequired === 'Excellent' ? 'bg-green-100 text-green-700' :
                      card.creditRequired === 'Good' ? 'bg-blue-100 text-blue-700' :
                      card.creditRequired === 'Fair' ? 'bg-yellow-100 text-yellow-700' :
                      card.creditRequired === 'Poor' ? 'bg-red-100 text-red-700' :
          ...
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div className="relative bg-white rounded-2xl max-w-md w-full p-6 shadow-xl transform transition-all">
              <div className="text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Delete Card</h3>
                <p className="text-gray-600 mb-6">
                  Are you sure you want to delete <span className="font-medium">{showDeleteModal.name}</span>? This action cannot be undone.
                </p>
